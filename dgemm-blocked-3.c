...
void multiply_2x1(double *A, double *B, double *C, int lda){
    __m128d AA, BB, CC;
    CC = _load128(C);
    for(int i = 0; i < lda; i++){
        AA = _load128(A + i * lda);
...
void multiply_2x4(double *A, double *B, double *C, int lda){
...
void multiply_4x4(double *A, double *B, double *C, int lda){
    ...
    for(int i = 0; i < lda; i++){
        ...
        BB1 = _mm256_set1_pd(B[i + 0 * lda]);
        BB2 = _mm256_set1_pd(B[i + 1 * lda]);
        BB3 = _mm256_set1_pd(B[i + 2 * lda]);
        BB4 = _mm256_set1_pd(B[i + 3 * lda]);
...
void multiply_8x4(double *A, double *B, double *C, int lda){
    encode_t AA1, AA2; 
    encode_t BB1, BB2, BB3, BB4;
...
void multiply_8x1(double *A, double *B, double *C, int lda){
...
void square_dgemm(int lda, double *A, double *B, double *C)
{
    ...
    for(int i = 0; i < lda / 8 * 8; i += 8){
        for(int j = 0; j < lda / 4 * 4; j += 4){
            multiply_8x4(A + i, B + j * lda, C + i + j * lda, lda);
        }
        for(int j = lda / 4 * 4; j < lda; j++){
            multiply_8x1(A + i, B + j * lda, C + i + j * lda, lda);
        }
    }
    ...
    for(int i = lda / 4 * 4; i < lda / 2 * 2; i += 2){
        for(int j = 0; j < lda / 4 * 4; j += 4){
            multiply_2x4(A + i, B + j * lda, C + i + j * lda, lda);
        }
        for(int j = lda / 4 * 4; j < lda; j++){
            multiply_2x1(A + i, B + j * lda, C + i + j * lda, lda);
        }
    }
